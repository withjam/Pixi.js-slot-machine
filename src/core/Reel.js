...
export default class Reel {
  constructor(turn) {
    ...
    this.delayStop = 0;
    this.singleReel = [
      { id: 0, texture: Loader.shared.resources['lowwin_club'].texture },
      { id: 1, texture: Loader.shared.resources['lowwin_diamond'].texture },
      { id: 2, texture: Loader.shared.resources['lowwin_heart'].texture },
      { id: 3, texture: Loader.shared.resources['lowwin_spade'].texture },
      { id: 4, texture: Loader.shared.resources['lowwin_star'].texture },
      { id: 5, texture: Loader.shared.resources['wild'].texture },
    ];
  ...
  createReels() {
    ...
    for (i = 0; i < this.singleReel.length; i++) {
      item = new Sprite(this.singleReel[i].texture);
  ...
  startAnimation(reel, callback) {
    ...
    if (!this.animation && selectedReel === this.height) {
      callback();
      return false;
    }
